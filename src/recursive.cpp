...
Ray generateReflectionRay(Ray ray, HitInfo hitInfo)
{
    ...
    glm::vec3 relfected = ray.direction - 2 * glm::dot(ray.direction, hitInfo.normal) * hitInfo.normal;
    Ray reflectedRay = Ray(ray.t * ray.direction + ray.origin + epsilon * glm::normalize(relfected), glm::normalize(relfected));
...
{
    float epsilon = 0.00001f;
    Ray passthroughRay = Ray(ray.t * ray.direction + ray.origin + epsilon * glm::normalize(ray.direction), ray.direction);
...
void renderRayTransparentComponent(RenderState& state, Ray ray, const HitInfo& hitInfo, glm::vec3& hitColor, int rayDepth)
{
    ...
    hitColor = glm::mix(passthroughColor, hitColor, hitInfo.material.transparency);
}