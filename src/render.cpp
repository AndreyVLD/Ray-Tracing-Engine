...
std::vector<Ray> generatePixelRaysMultisampled(RenderState& state, const Trackball& camera, glm::ivec2 pixel, glm::ivec2 screenResolution)
{
    ...
    for (int i = 0; i< numSamples; i++) {
        float pozx = pixel.x + state.sampler.next_1d();
        float pozy = pixel.y + state.sampler.next_1d();

        glm::vec2 position = (glm::vec2(pozx,pozy)) / glm::vec2(screenResolution) * 2.f - 1.f;
...
std::vector<Ray> generatePixelRaysStratified(RenderState& state, const Trackball& camera, glm::ivec2 pixel, glm::ivec2 screenResolution)
{
    ...
    std::vector<Ray> rays;
    float binsize = 1.f / numSamples;
    for (int  i=0; i<numSamples;i++)
    {
		for (int j=0; j<numSamples;j++)
		{
            float pozx = pixel.x + binsize * i + state.sampler.next_1d()*binsize;
            float pozy = pixel.y + binsize * j + state.sampler.next_1d()*binsize;
